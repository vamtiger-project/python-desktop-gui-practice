...
from PyQt4.QtCore import (QByteArray, QDataStream, QFile, QFileInfo,
						  QIODevice, QPoint, QPointF, QRectF, QString,
						  Qt, SIGNAL
						 )
from PyQt4.QtGui import (QApplication, QCursor, QDialog, QDialogButtonBox,
						 QFileDialog, QFont, QFontComboBox, QFontMetricsF, QFontMetrics,
						 QGraphicsItem, QGraphicsPixmapItem, QGraphicsScene,
						 QGraphicsTextItem, QGraphicsView, QGridLayout,
						 QHBoxLayout, QLabel, QMatrix, QMenu, QMessageBox,
						 QPainter, QPen, QPixmap, QPrintDialog, QPrinter,
						 QPushButton, QSpinBox, QStyle, QTextEdit, QVBoxLayout
						)
...
class MainForm(QDialog):
	...
	def __init__(self, parent = None):
		...
		self.resize(self.scene.width() + fm.width(" Delete... ") + 50, self.scene.height() + 50)
	...
	def addPixmap(self):
		...
		fname = QFileDialog.getOpenFileName(self, "Page Designer - Add Pixmap", "Pixmap Files (*.bmp *.jpg *.png *.xpm)")
		if fname.isEmpty():
			return
		else:
	...
	def delete(self):
		...
		if (len(items) and QMessageBox.question(self,
												   ...
												   "Delete {0} item{1}?".format(len(items), "s" if len(items) != 1 else ""),
	...
	def open(self):
		self.offerSave()
		path = (QFileInfo(self.filename).path() if not self.filename.isEmpty() else "")
		fname = QFileDialog.getOpenFileName(self, "Page Designer - Open",
											path, "Page Designer Files (*.pgd)")
		if fname.isEmpty():
			return
		self.filename = fname
		fh = None
		try:
			...
			if fileVersion != FileVersion:
				raise IOError, "unrecognized .pgd file version"
			while not fh.atEnd():
				self.readItemFromStream(stream)
		except IOError, e:
			QMessageBox.warning(self, "Page Designer -- Open Error"
								"Falied to open {0}: {1}".format(self.filename, e))
		finally:
			if fh is not None:
				fh.close()
		global Dirty
	...
	def offerSave(self):
		if (Dirty and QMessageBox.question(self, "Page Designer - Usaved Changes",
...
class BoxItem(QGraphicsItem):
	...
	def __init__(self, position, scene, style = Qt.SolidLine, rect = None, matrix = QMatrix()):
		...
		if rect is None:
			rect = QRectF(-10 * PointSize, -PointSize, 20 * PointSize, 2 * PointSize)
	...
	def contextMenuEvent(self, event):
		...
		for text, param in (("&Solid", Qt.SolidLine),
							("&Dashed", Qt.DashLine),
							("D&otted", Qt.DotLine),
...
class TextItemDlg(QDialog):
	...
	def updateUi(self):
		...
		self.buttonBox.button(QDialogButtonBox.Ok).setEnabled(not self.editor.toPlainText().isEmpty())
