...
from PyQt4.QtCore import (QEvent, QTimer, SIGNAL, Qt, QString)
...
class Widget(QWidget):
	...
	def resizeEvent(self, event):
		self.text = QString("Resized to QSize({0}, {1})".format(event.size().width(), event.size().height()))
		self.update()
	...
	def mouseMoveEvent(self, event):
		if not self.justDoubleClicked:
			...
			self.text = QString("The mouse is at\nQPoints({0}, {1}) "
								...
								"QPoint({2}, {3}) in screen coords".format(event.pos().x(), event.pos().y(), globalPos.x(), globalPos.y()))
			self.update()
