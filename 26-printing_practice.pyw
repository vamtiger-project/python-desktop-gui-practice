...
from PyQt4.QtGui import (QApplication, QDialog, QFont, QFontMetrics,
						 QHBoxLayout, QPainter, QPixmap, QPrintDialog,
						 QPrinter, QPushButton, QTableWidget, QTableWidgetItem,
...
class Form(QDialog):
	...
	def __init__(self, parent = None):
		...
		paintButton = QPushButton("Print via Q&Painter")
		paintButton.clicked.connect(self.printViaQPainter)
		...
		buttonLayout.addWidget(paintButton)
	...
	def generateFakeStatements(self):
		...
		statement = Statement("Consality", "Ms S. Royal", "234 Rue Saint Hyacinthe, 75021, Paris")
		...
		statement = Statement("Demamitur Plc", "Mr G. Brown", "14 Tall Towers, Tower Hamlets, London, WC1 3BX")
	...
	def populateTable(self):
		...
		for row, statement in enumerate(self.statements):
			...
			item = QTableWidgetItem(QString("$ %L1").arg(float(statement.balance()), 0, "f", 2))
	...
	def printViaQCursor(self):
		...
		for statement in self.statements:	
			...
			for text in ("Greasy Hands Ltd.", "New Lambard Street", "London", 
						 "WC13 4PX", QDate.currentDate().toString(DATE_FORMAT)
						):
				cursor.insertBlock(headFormat, headCharFormat)
			...
			cursor.insertText(QString("The balance of your account is $ %L1.").arg(float(balance), 0, "f", 2))
			...
			for date, amount in statement.transactions:
				...
				cellCursor.insertText(QString("$ %L1").arg(float(amount), 0, "f", 2), format)
	...
	def printViaHtml(self):
		...
		for statement in self.statements:
			...
			html += ("<p align = right><img src = ':/logo.png'></p>"
					...
					).format(date, address, contact, QString("$ %L1").arg(float(balance), 0, "f", 2))
			...
			html += ("</p><p>&nbsp;</p>"
					 ...
					 "<tr><td colspan = 3>Transactions:</td></tr>"
					)
	...
	def printViaQPainter(self):
		...
		for statement in self.statements:
			...
			painter.drawText(x, y, "New Lambard Street")
			...
			painter.drawText(x, y, QString("The balance of your account is $ %L1").arg(float(balance), 0, "f", 2))
			...
			for date, amount in statement.transactions:
				...
				painter.drawText(QRectF(x + 3, y + 3, DateWidth - 6, h - 3), date.toString(DATE_FORMAT), option)
				...
				painter.drawText(QRectF(x + 3, y + 3, CreditWidth - 6, h - 3), text, option)
				...
				painter.drawText(QRectF(x + 3, y + 3, AmountWidth - 6, h - 3), QString("$ %L1").arg(float(amount), 0, "f", 2), option)
			...
			painter.drawText(QRectF(x, y, pageRect.width() - 2 * LeftMargin, 31),
							 "The contents of this letter are for information only and do not form part of any contract",
							 option
							)
