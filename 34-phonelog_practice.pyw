...
from PyQt4.QtGui import (QApplication, QComboBox, QCursor,
						 QDataWidgetMapper, QDateTimeEdit,
						 QDialog, QGridLayout, QHBoxLayout,
						 QIcon, QLabel, QLineEdit, QMessageBox,
						 QPixmap, QPushButton, QVBoxLayout
						)
...
class PhoneLogDlg(QDialog):
	...
	def __init__(self, parent = None):
		...
		self.model = QSqlRelationalTableModel(self)	#For tables with foreign keys
...
def main():
	...
	def createFakeData():
		...
		pics = ("Complaint", "Information request", "Off topic",
		...
		query.exec_("SELECT id, caller, starttime, endtime, topic, outcomeid FROM calls ORDER by starttime")

		while query.next():
			id = query.value(ID).toInt()[0]
			caller = unicode(query.value(CALLER).toString())
			starttime = (unicode(query.value(STARTTIME).toDateTime().toString(DATETIME_FORMAT)))
			endtime = (unicode(query.value(ENDTIME).toDateTime().toString(DATETIME_FORMAT)))
			topic = unicode(query.value(TOPIC).toString())
			outcomeid = query.value(OUTCOMEID).toInt()[0]
			subquery = QSqlQuery("SELECT name FROM outcomes WHERE id = {0}".format(outcomeid))
			outcome = "invalid foreign key"

			if subquery.next():
				outcome = unicode(subquery.value(0).toString())

			print("{0:02d}: {1} {2} - {3} {4} [{5}]".format(id, caller, starttime, endtime, topic, outcome))
	...
	if create:
		...
		splash = QLabel()
		splash.setPixmap(QPixmap(":/phonlogsplash.png"))
