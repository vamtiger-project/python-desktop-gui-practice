...
from PyQt4.QtGui import (QApplication, QCursor, QPainter, QPen, QPixmap,
						 QSizePolicy, QWidget)
...
class CountersWidget(QWidget):
	...
	def keyPressEvent(self, event):
		if event.key() == Qt.Key_Left:
			self.selected[0] = 2 if self.selected[0] == 0 else self.selected[0] - 1
		elif event.key() == Qt.Key_Right:
			self.selected[0] = 0 if self.selected[0] == 2 else self.selected[0] + 1
		elif event.key() == Qt.Key_Up:
			self.selected[1] = 2 if self.selected[1] == 0 else self.selected[1] - 1
		elif event.key() == Qt.Key_Down:
			self.selected[1] = 0 if self.selected[1] == 2 else self.selected[1] + 1
	...
	def paintEvent(self, event = None):
		...
		for x in range(3):
			for y in range(3):
				cell = self.grid[x][y]
				rect = QRectF(x * xOffset, y * yOffset, xOffset, yOffset).adjusted(0.5, 0.5, -0.5, -0.5)
				color = None
				if cell == RED:
					color = Qt.red
				elif cell == YELLOW:
