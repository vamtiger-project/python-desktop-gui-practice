...
from PyQt4.QtGui import (QApplication , QMainWindow, QAction, QIcon, QListWidget,
...
class MainWindow(QMainWindow):
	...
	def createMenusAndToolbars(self):
		...
		for icon, text in (("new", "&New..."), ("open", "&Open..."),
						   ...
						   (None, None), ("quit", "&Quit")):
			if icon is None: fileMenu.addSeparator()
			else:
				action = QAction( QIcon(":/file{0}.png".format(icon)), text, self )
				if icon == "quit": self.connect(action, SIGNAL("triggered()"), self.close)
				elif text != "Save &As...": fileToolbar.addAction(action)
		...
		for icon, text in (("add", "&Add..."), ("edit", "&Edit..."),
						   ("delete", "&Remove")):
			action = QAction( QIcon(":/edit{0}.png".format(icon)), text, self )
