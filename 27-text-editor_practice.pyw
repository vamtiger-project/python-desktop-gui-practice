...
from PyQt4.QtGui import (QApplication, QAction, QColor, QFileDialog,
						 QFont, QIcon, QKeySequence, QMainWindow, QMessageBox,
						 QSyntaxHighlighter, QTextCharFormat, QTextEdit)
...
class MainWindow(QMainWindow):
	...
	def __init__(self, filename=None, parent=None):
		...
		fileNewAction = self.createAction("&New...", self.fileNew, QKeySequence.New,
										  "filenew", "Create a python file")
		fileOpenAction = self.createAction("&Open...", self.fileOpen, QKeySequence.Open,
										   "fileopen", "Open an existing Python file")
		self.fileSaveAction = self.createAction("&Save", self.fileSave, QKeySequence.Save,
												"filesave", "Save the file")
		self.fileSaveAsAction = self.createAction("Save &As...", self.fileSaveAs,
												  icon = "filesaveas", tip = "Save the file using a new file name")
		fileQuitAction = self.createAction("&Quit", self.close, "Ctrl+Q",
										   "filequit", "Close the application")
		self.editCopyAction = self.createAction("&Copy", self.editor.copy, QKeySequence.Copy,
												"editcopy", "Copy text to the clipboard")
		self.editCutAction = self.createAction("Cu&t", self.editor.cut, QKeySequence.Cut,
											   "editcut", "Cut text to the clipboard")
		self.editPasteAction = self.createAction("&Paste", self.editor.paste, QKeySequence.Paste,
		...
		self.connect(self.editor, SIGNAL("selectionChanged()"), self.updateUi)
		self.connect(self.editor.document(), SIGNAL("modificationChanged(bool)"), self.updateUi)
		self.connect(QApplication.clipboard(), SIGNAL("dataChanged()"), self.updateUi)
	...
	def fileNew(self):
		...
		self.filename = None
		self.setWindowTitle("Python Editor - Unamed")
		self.updateUi()

	def okToContinue(self):
		if self.editor.document().isModified():
			reply = QMessageBox.question(self, "Python Editor - Usaved Changes",
	...
	def fileSaveAs(self):
		...
		filename = QFileDialog(self, "Python Editor -- Save File As",
	...
	def fileOpen(self):
		...
		fname = unicode(QFileDialog.getOpenFileName(self, "Python Editor - Choose File", dir, "Python Files (*.py *.pyw)"))
		if fname:
			self.filename = fname
			self.loadFile()
