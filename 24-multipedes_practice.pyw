...
from PyQt4.QtGui import (QApplication, QBrush, QColor, QDialog,
						 ...
						 QHBoxLayout, QPainter, QPainterPath, QPolygonF,
...
class MainForm(QDialog):
	...
	def __init__(self, parent = None):
		...
		self.connect(quitButton, SIGNAL("clicked()"), self.reject)
	...
	def timeout(self):
		...
		for item in self.scene().collidingItems(self):
			if isinstance(item, Head):
				...
				item.color.setBlue(min(255, self.color.blue() + 1))
...
class Segment(QGraphicsItem):
	...
	def __init__(self, color, offset, parent):
		...
		self.path.addPolygon(QPolygonF([QPointF(x, y), QPointF(x - 5, y - 12), QPointF(x - 5, y)]))
